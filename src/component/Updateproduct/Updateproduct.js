...
function Updateproduct() {
  ...
  const [reference,setreference]=useState(product?product[0].maincategory:"")
  ...
  const [products,setproducts]=useState()
    ...
    useEffect(()=>{
      async function fetch(){
           const res=await axios.get("http://localhost:8000/category/getall",{headers:{
  ...
  useEffect(() => {
    async function fetchdatas() {
      const responses = await axios.get(`http://localhost:8000/product/${Id.id}`,{
        headers: {
          Authorization: user.token,
        },
      });
      setproduct(responses.data.message);
      console.log(responses.data.message[0].category)
      setcategories(responses.data.message[0].category)
  ...
  const fetchdata = async () => {
    const responses = await Promise.all(
      categories.map(async (c) => {
        try {
          const response = await axios.get(`http://localhost:8000/category/${c}`, {
            headers: {
              Authorization: user.token,
            },
          });
    ...
    newproduct.photo = [fileName1, fileName2, fileName3];
      ...
      try{
        await axios.post("http://localhost:8000/upload/image1", data1)
        await axios.post("http://localhost:8000/upload/image2", data2)
        await axios.post("http://localhost:8000/upload/image3", data3)
      ...
      try{
        await axios.post('http://localhost:8000/product/create',newproduct, {headers: {
          Authorization: user.token,
        },
      })
  ...
  return (
    <div className='p-5'>
       ...
       <input className='form-control mb-3' type='text' placeholder='Product name' defaultValue={product?product[0].username:""}  ref={productname} required/>
       <div className="row mb-3 mt-3">
       
  <div className="col-lg-6">
  <select class="form-control mb-3" ref={maincategory}  onChange={(e)=>{setreference(e.target.value)}}>
    {product?<option>{product[0].maincategory}</option>:<option>Watch</option>}
    <option>Watch</option>
    <option>Cloth</option>
    <option>Shoe</option>
  </select>
  ...
  <div className="col">
    <input type="text" className="form-control" defaultValue={product?product[0].price:""} placeholder="Price"   ref={price} required />
  </div>
</div>
<div className='row mb-3'>
<select class="form-control mb-3 col-lg-7 mr-3 ml-3" ref={category} defaultValue={product?product[0].category:""}>
{
  item.map((i)=>(
...
<div className='row mb-3'> 
     <div className='col-lg-3'>
       <input className='form-control' placeholder='items in size "SMALL" ' ref={quantity1} defaultValue={product?product[0].quantity1:""}/>
     </div>
     <div className='col-lg-3'>
     <input className='form-control' placeholder='items in size "MEDIUM" 'ref={quantity2} defaultValue={product?product[0].quantity2:""} />
     </div>
     <div className='col-lg-3'>
     <input className='form-control' placeholder='items in size "LARGE" ' ref={quantity3} defaultValue={product?product[0].quantity3:""}/>
     </div>
  </div>
</> :
<>
<div className='mb-3 col-lg-8'>
     <input className='form-control' placeholder='Enter the no:of items' ref={quantity} defaultValue={product?product[0].quantity:""}/>
     </div>
  </>
